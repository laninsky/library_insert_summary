...
# logging whether reads map to the same contig or not
combined_table <- mutate(combined_table, same_contig = ifelse(X1==X4, "Same", "Different"))

# Summarizing the number of library pairs found on the same contig or not
write("Number of read pairs found on same or different contigs:","library_insert_summary.log",append=TRUE)
write_tsv(count(combined_table, same_contig),"library_insert_summary.log",append=TRUE)
write("","library_insert_summary.log",append=TRUE)

# Of those found on the same contig, getting the internal insert size between the reads
combined_table <- filter(combined_table, same_contig=="Same")
combined_table <- mutate(combined_table, insert_size = pmin(abs(X2-X5),abs(X2-X6),abs(X3-X5),abs(X3-X6)))

write("For read pairs found on the same contig...","library_insert_summary.log",append=TRUE)
...
write("Length distribution of insert sizes for read pairs found on same contig:","library_insert_summary.log",append=TRUE)
